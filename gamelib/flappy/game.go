...
func init() {
	img, _, err := image.Decode(bytes.NewReader(rflappy.Gopher_png))
	if err != nil {
		gframe.Log().Fatal(err)
	}
...
func init() {
	tt, err := opentype.Parse(rfonts.PressStart2P_ttf)
	if err != nil {
		gframe.Log().Fatal(err)
	}
	const dpi = 72
...
func (g *Game) isKeyJustPressed() bool {
	...
	g.touchIDs = inpututil.AppendJustPressedTouchIDs(nil)
	if len(g.touchIDs) > 0 {
		return true
	}
